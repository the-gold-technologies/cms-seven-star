...
import { CloudUpload, Trash2, Sparkles, Plus, Image as ImageIcon } from "lucide-react";
...
export function FeatureTilesSection({
  ...
  initialData,
  saveUrl = "/api/home",
  responseKey = "Integrations", // Map to Integrations DB slot to prevent schema breakage
  onSave,
  ...
  const handleCardFieldChange = (fIdx: number, field: "title" | "description", value: string) => {
    setFormData((prev) => {
  ...
  const handleFileChange = (fIdx: number, sIdx: number, e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleSave = async () => {
    ...
    formData.features.forEach((feat, fIdx) => {
      if (!feat.title?.trim()) errs.push(`Feature ${fIdx + 1} Title is required`);
      if (tileImages[fIdx].some((img) => !img)) {
        errs.push(`All 3 slide images are required for Card ${fIdx + 1} (${feat.title})`);
      }
    });
    ...
    try {
      ...
      const finalFeatures = formData.features.map((feat, fIdx) => {
        const finalImages = feat.images.map((_, sIdx) => {
          ...
          const url = cell instanceof File ? uploadedUrls[uploadIndex] || "" : cell;
          uploadIndex++;
          return url;
        });
        return {
      ...
      if (json.success) {
        toast.success("Feature Tiles section saved successfully!", { id: toastId });
              ...
              <div className="flex flex-col gap-6">
                ...
                <div className="flex flex-col gap-8 mt-2">
                  {formData.features.map((feat, fIdx) => (
                    <div key={fIdx} className="flex flex-col gap-4 bg-gray-50/40 p-6 border border-gray-100 rounded-3xl shadow-sm relative">
                      <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block">Card {fIdx + 1}: {feat.title || "Tradition Card"}</span>
                      
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <InputField
                          label="Tradition Title"
                          value={feat.title}
                          onChange={(e) => handleCardFieldChange(fIdx, "title", e.target.value)}
                        ...
                        <TextAreaField
                          ...
                          onChange={(e) => handleCardFieldChange(fIdx, "description", e.target.value)}
                              ...
                              <div key={sIdx} className="flex flex-col gap-2">
                                ...
                                {preview ? (
                                  <div className="flex items-center justify-between p-3.5 px-5 bg-white border border-gray-200 rounded-2xl transition-all hover:bg-gray-50/50">
                                    <div className="flex items-center gap-3.5 text-gray-700">
                                      <div className="w-8 h-8 rounded-lg overflow-hidden bg-gray-200 border border-gray-300/40 relative flex-shrink-0">
                                        ...
                                        <img src={preview} alt={`${feat.title} slide ${sIdx + 1}`} className="w-full h-full object-cover" />
                                      </div>
                                      <div className="flex flex-col">
                                        ...
                                        <span className="text-[9px] text-gray-400 font-semibold mt-0.5">
                                          {feat.title || "Tradition"} - Slide Position {sIdx + 1}
                                        </span>
                                      </div>
                                    </div>
                                    <div className="flex items-center gap-2">
                                      <button
                                        type="button"
                                        onClick={() => fileInputRefs.current[fIdx][sIdx]?.click()}
                                  ...
                                  >
                                    ...
                                    <p className="text-[10px] text-gray-400 mt-1">PNG, JPG or WEBP (Slide {sIdx + 1})</p>
                                  </div>
                                )}
                                <input
                                  type="file"
                                  ref={(el) => {
                                    if (!fileInputRefs.current[fIdx]) fileInputRefs.current[fIdx] = [];
                                    fileInputRefs.current[fIdx][sIdx] = el;
                                  }}
                                  onChange={(e) => handleFileChange(fIdx, sIdx, e)}
                                  accept="image/*"
                                  className="hidden"
                                />
                              </div>
