...
export function StoryHeroCMS({
  sectionId,
  initialData,
  saveUrl = "/api/our-story",
  responseKey = "StoryHero",
  onSave,
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    const toastId = toast.loading("Saving Story Hero...");
    try {
      ...
      if (json.success) {
        toast.success("Story Hero saved successfully!", { id: toastId });
        setFormData(payload);
  ...
  const name = typeof selectedImage === "string" ? selectedImage.split("/").pop() || "Cover Image" : selectedImage?.name;

  return (
    <section>
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 flex flex-col gap-4 transition-all">
        <SectionHeader
          title="Story Hero Section"
          description="Manage main heritage background parallax photos, heritage tags, headings, and descriptions."
          isOpen={isOpen}
            ...
            <div className="flex flex-col gap-8 pt-6 animate-in fade-in duration-500">
              ...
              <div className="flex flex-col gap-6 bg-gray-50/20 border border-gray-100 p-6 rounded-2xl w-full">
                <InputField
                  ...
                  placeholder="e.g. Heritage & People"
                  required
                  ...
                  <div className="flex items-center justify-between p-3.5 px-5 bg-white border border-gray-200 rounded-2xl transition-all hover:bg-gray-50/50 mt-1">
                    <div className="flex items-center gap-3.5 text-gray-700">
                      <div className="w-8 h-8 rounded-lg overflow-hidden bg-gray-200 border border-gray-300/40 relative flex-shrink-0">
                        ...
                        <img src={preview} alt="Story Hero BG" className="w-full h-full object-cover" />
                      </div>
